...
export class SignatureMockRepository implements SignatureRepositoryInterface {
  ...
  async create(plan_id: number,user_id: number,period: number,due_day: number,): Promise<SignatureEntity> {
  ...
  async updateDueDay(signatureEntity: SignatureEntity): Promise<SignatureEntity> {
